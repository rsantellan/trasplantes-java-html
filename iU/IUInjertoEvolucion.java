...
public class IUInjertoEvolucion extends JFrame {
	...
	public IUInjertoEvolucion(int idPretrasplante, EvolucionInjerto e,
		...
		if (e != null) {
			this.cargar(e);
			this.viejo = e;
			if (!ver)this.jLabelFechaOK.setVisible(true);
			
		}
	}

	public IUInjertoEvolucion(int idPretrasplante,Calendar fecha, EvolucionInjerto e,
			boolean ver, boolean esTrasplante) {
		super();
		this.fecha = fecha;
		initialize();
		...
		if (e != null) {
			...
			if(esTrasplante) this.jButtonEliminar.setVisible(true);
		}
	}
	...
	private void cargarDatosFijos() {
		...
		ArrayList<RaTratamiento> listaRa = Fachada.getInstancia().obtenerTodosRaTratamiento();
	...
	private void initialize() {
		...
		if(this.fecha !=null){
			this.setTitle(this.getTitle() + " la fecha del trasplante es :"+ManejoFechas.formatoEspanol.format(this.fecha.getTime()));
		}
	}
	...
	private void guardar() {
		...
		if (this.viejo == null) {
			if(this.fecha == null){
				fechaOK = Fachada.getInstancia().fechaEvolucionInjertoCorrecta(this.idPreTrasplante,this.jDateChooserFechaControl.getCalendar());
			}else{
				...
				if(this.jDateChooserFechaControl.getCalendar().before(aux)){
					...
					fechaOK = false;
					JOptionPane.showMessageDialog(this,"La fecha tiene que estar comprendida entre la fecha del trasplante y un mes despues de este." , "Atencion", 2);
				}
			}
		}
		if (fechaOK) {
			...
			aux.setTratamiento((RaTratamiento)this.jComboBoxTratamiento.getSelectedItem());
	...
	private void eliminar(){
		...
		respuesta = JOptionPane
				.showInputDialog(
						this,
						"Está a punto de eliminar una complicacion no infecciosa\n(Ingrese SI para eliminar) Confirmar: ",
						"Confirmar", 1);
		if (respuesta == null)
			respuesta = "No";
