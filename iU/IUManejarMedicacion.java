...
public class IUManejarMedicacion extends JFrame implements java.util.Observer {
	...
	private ArrayList<Medicacion> lista;  //  @jve:decl-index=0:
	/**
	 * This is the default constructor
	 */
	public IUManejarMedicacion() {
		super();
		initialize();
		cargarDatos();
	...
	private void cargarDatos(){
		this.lista = Fachada.getInstancia().obtenerTodasMedicaciones();
	...
	private void initialize() {
		...
		this.setTitle("Manejar Tipos de Medicacion");
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jLabelTiposInduccion.setText("Tipos de Medicacion");
	...
	private void agregar(){
		String nueva = "";
		nueva = JOptionPane.showInputDialog(this, "Ingrese el nombre de la nueva Medicacion :", "Confirmar", 3);
		if(nueva != null){
			if(!nueva.equalsIgnoreCase("")|| !nueva.equalsIgnoreCase(" ")){
				Medicacion m = new Medicacion();
				m.setNombre(nueva);
				if(this.lista.contains(m)){
					JOptionPane.showMessageDialog(this,"Ese tipo de Medicacion ya se encuentra ingresado" , "Error", 2);
				}else{
					Fachada.getInstancia().guardarMedicacion(m);
				}
			}
		}
	}
	...
	private void modificar(){
		...
		if(m!= null){
			JOptionPane.showMessageDialog(this,"Todas las complicaciones que esten asociados con esta medicacion se veran afectados" , "Atencion", 2);
			String nueva = "";
			nueva = JOptionPane.showInputDialog(this, "Ingrese el nuevo nombre de la Medicacion :", "Confirmar", 3);
			if(nueva != null){
				if(!nueva.equalsIgnoreCase("")|| !nueva.equalsIgnoreCase(" ")){
					Medicacion aux = new Medicacion();
					aux.setNombre(nueva);
					if(this.lista.contains(aux)){
						JOptionPane.showMessageDialog(this,"La Medicacion que quiere ingresar ya existe" , "Error", 2);
					}else{
						m.setNombre(nueva);
						Fachada.getInstancia().guardarMedicacion(m);
					}
	...
	private void eliminar(){
		...
		if(m!= null){
			...
			if(!eliminar){
				JOptionPane.showMessageDialog(this,"Existe por lo una complicacion con esta medicacion.\nPor lo tanto no se puede borrar." , "Error", 2);
			}else{
				Fachada.getInstancia().eliminarMedicacion(m);
			}
		}
	}
