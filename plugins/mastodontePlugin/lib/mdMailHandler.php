...
class mdMailHandler {
    ...
    static public function sendConfirmationMail(mdPassport $mdPassport, $realtime = true, $removeBackendControler = true){
        ...
        $link = url_for('@confirm_user_password',true);
        $link .= '?confirmation=' . $params;
        if($removeBackendControler)
        {
    ...
    public static function removeBackendControler($url)
    {
        ...
        sfContext::getInstance ()->getLogger()->err('<<<!!!!!'.$url);
        sfContext::getInstance ()->getLogger()->err('<<<!!!!!'.$new_url);
        return $new_url;
    }
    ...
    //static public function sendMdContactMail($body, $subject, $from, $sendMe = false)
		 ...
		 * @author maui .-
		 */
		public static function sendSwiftMail($from, $to, $subject, $body, $needReply = false, $replyTo = "", $attachMents = array(),$realtime = true)
    {
    ...
    static public function sendPhpMail($recipients, $from, $subject, $body)
    {
	        ...
	        $headers .= "\r\n";
	        // Mail it
	        if (!mail($recipient, $subject, $body, $headers)) return false;
				}
				return true;
		...
		static public function sendMail($options)
		{
			...
			if(!isset($options['recipients'])) throw new Exception('Envio de mail sin destinatario');
			else{
			...
			if(!isset($options['sender'])) throw new Exception('Envio de mail sin datos de envío');
			else
				$sender = $options['sender'];


			if(!isset($options['subject'])) throw new Exception('Envio de mail sin asunto');
			else
				$subject = $options['subject'];

			if(!isset($options['body'])) throw new Exception('Envio de mail sin cuerpo');
			else
				$body = $options['body'];

			$replyTo = null;
			if(isset($options['replyTo'])) $replyTo = $options['replyTo'];

			$attachments = null;
			if(isset($options['attachments'])) $attachments = $options['attachments'];

			if(isset($options['usePhpMail'])) $sendUsingPhpMail = $options['usePhpMail'];
			else $sendUsingPhpMail = sfConfig::get('app_php_mailer_enabled', false);
