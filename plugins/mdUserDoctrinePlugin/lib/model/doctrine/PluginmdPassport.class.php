...
abstract class PluginmdPassport extends BasemdPassport
{
		...
		if (!$mdPassport->isActive()) {
            ...
            if ($mdPassport->getPassword() === md5($pass)) {
                if( sfConfig::get( 'sf_plugins_user_groups_permissions', false ) )
                {
                    ...
                    {
                        self::checkIfMdPassportHaveGroups($mdPassport->getId());
                    }
                }
                return $mdPassport;
            } else {
    ...
    private static function checkIfMdPassportHaveGroups($mdPassportId)
    {
    ...
    */
    ...
    public function loadGroupsAndPermissions() {
        ...
        if (!isset($this->_permissions)) {
            $permissions = Doctrine::getTable('mdPermission')->getPermissionOfUserOnApplication($this->getId(), $appName);
            $storage_permission = array();
            foreach ($permissions as $permission) {
                $storage_permission[$permission->getName()] = $permission;
            }
            $this->_permissions[$appName] = $storage_permission;
        }
        
        if (!isset($this->_groups[$appName])) {
            $groups = Doctrine::getTable('mdGroup')->getGroupsOfUserOnApplication($this->getId(), $appName);
            $storage_groups = array();
            foreach ($groups as $group) {
                $storage_groups[$group->getName()] = $group;
            }
            $this->_groups[$appName] = $storage_groups;
        }
        */
    }
