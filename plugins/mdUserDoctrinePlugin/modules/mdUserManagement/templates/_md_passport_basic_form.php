<div id="md_user_md_passport_edit_div_<?php echo $mdPassportForm->getObject()->getId() ?>">
    ...
    <?php if($sf_user->getMdPassport()->retrieveMdUser()->getSuperAdmin()):?>
            <div class="md_blocks">
                ...
                <div >
                                <input type="checkbox" onclick="mdUserManagement.getInstance().changeMdUserSuperAdmin(<?php echo $mdPassportForm->getObject()->getMdUserId()?>)"<?php  if($mdPassportForm->getObject()->retrieveMdUser()->getSuperAdmin()) echo 'checked';?>/>
                </div>
            </div>
    <?php endif;?>
</div>
