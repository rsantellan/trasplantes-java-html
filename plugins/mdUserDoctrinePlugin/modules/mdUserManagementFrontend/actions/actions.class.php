...
/**
    ...
    public function executeRetrieveLastUploadImageAsAvatar(sfWebRequest $request) {
        ...
        $width = $request->getParameter("width", 83);
        $height = $request->getParameter("height", 71);
        $object = Doctrine::getTable($class)->find($id);
        if($object)
        {
            $instance = mdMediaManager::getInstance(mdMediaManager::MIXED, $object)->load("default");
            ...
            $body = $this->getPartial("mdUserManagementFrontend/avatar_image", array("width" => $width, "height" => $height));
            return $this->renderText(mdBasicFunction::basic_json_response(true, array('body'=>$body)));
        }
        else
        {
