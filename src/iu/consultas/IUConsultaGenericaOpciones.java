...
public class IUConsultaGenericaOpciones extends JFrame {
	 ...
	 */
	public IUConsultaGenericaOpciones() {
		super();
		initialize();
		cargarDatos();
	}

	public IUConsultaGenericaOpciones(int id) {
		super();
		this.id = id;
		initialize();
		cargarDatos();
	...
	private void cargarDatos(){
		...
		switch (id) {
		case 0:
			g = Fachada.getInstancia().consultaPacientesReoperaciones();
			this.jLabelSeleccione.setText("PACIENTES CON REOPERACIONES");
			break;
	...
	private void masDatos(){
		...
		if(Integer.parseInt(this.usado.getListaObtenida().get(lugar + 4).toString()) > 0){
			...
			System.out.println(this.usado.getListaObtenida().get(lugar + 1).toString());
		}
