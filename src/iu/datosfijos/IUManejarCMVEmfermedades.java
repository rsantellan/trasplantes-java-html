...
public class IUManejarCMVEmfermedades extends JFrame implements java.util.Observer {
	...
	private ArrayList<CMVEnfermedades> lista;  //  @jve:decl-index=0:
	/**
	 * This is the default constructor
	 */
	public IUManejarCMVEmfermedades() {
		super();
		initialize();
		cargarDatos();
	...
	private void cargarDatos(){
		this.lista = Fachada.getInstancia().obtenerTodasCMVEmfermedades();
	...
	private void initialize() {
		...
		this.setTitle("Manejar Tipos de Emfermedades de CMV");
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jLabelTiposInduccion.setText("Tipos de Emfermedades CMV");
	...
	private void agregar(){
		String nueva = "";
		nueva = JOptionPane.showInputDialog(this, "Ingrese el nombre de la nueva Emfermedad CMV :", "Confirmar", 3);
		if(nueva != null){
			if(!nueva.equalsIgnoreCase("")|| !nueva.equalsIgnoreCase(" ")){
				CMVEnfermedades m = new CMVEnfermedades();
				m.setNombre(nueva);
				if(this.lista.contains(m)){
					JOptionPane.showMessageDialog(this,"Ese tipo de emfermedad de CMV ya se encuentra ingresado" , "Error", 2);
				}else{
					Fachada.getInstancia().guardarEmfermedadCMV(m);
				}
			}
		}
	}
	...
	private void modificar(){
		CMVEnfermedades m = (CMVEnfermedades) this.jListMedicacion.getSelectedValue();
		if(m!= null){
			JOptionPane.showMessageDialog(this,"Todas las complicaciones que esten asociados con esta Emfermedad CMV se veran afectados" , "Atencion", 2);
			String nueva = "";
			nueva = JOptionPane.showInputDialog(this, "Ingrese el nuevo nombre de la Emfermedad CMV :", "Confirmar", 3);
			if(nueva != null){
				if(!nueva.equalsIgnoreCase("")|| !nueva.equalsIgnoreCase(" ")){
					CMVEnfermedades aux = new CMVEnfermedades();
					aux.setNombre(nueva);
					if(this.lista.contains(aux)){
						JOptionPane.showMessageDialog(this,"La Emfermedad CMV que quiere ingresar ya existe" , "Error", 2);
					}else{
						m.setNombre(nueva);
						Fachada.getInstancia().guardarEmfermedadCMV(m);
					}
	...
	private void eliminar(){
		...
		if(m!= null){
			String respuesta = "";
			respuesta = JOptionPane.showInputDialog(this, "Está a punto de eliminar una Emfermedad de CMV\n(Ingrese SI para eliminar) Confirmar: ", "Confirmar", 1);
			if(respuesta==null)respuesta="No";
			if(respuesta.equalsIgnoreCase("Si")){
				boolean eliminar = Fachada.getInstancia().verificarEliminacionCMVEnfermedad(m);
				if(!eliminar){
					JOptionPane.showMessageDialog(this,"Existe por lo una CMV con esta Emfermedad CMV.\nPor lo tanto no se puede borrar." , "Error", 2);
				}else{
					Fachada.getInstancia().eliminarEmfermedadCMV(m);
				}
			}
