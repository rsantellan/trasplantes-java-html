...
public class IUManejarCMVDrogas extends JFrame implements java.util.Observer {
	...
	private ArrayList<CMVDrogas> lista;  //  @jve:decl-index=0:
	/**
	 * This is the default constructor
	 */
	public IUManejarCMVDrogas() {
		super();
		initialize();
		cargarDatos();
	...
	private void cargarDatos(){
		this.lista = Fachada.getInstancia().obtenerTodasCMVDrogas();
	...
	private void initialize() {
		...
		this.setTitle("Manejar Tipos de Drogas de CMV");
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jLabelTiposInduccion.setText("Tipos de Drogas CMV");
	...
	private void agregar(){
		String nueva = "";
		nueva = JOptionPane.showInputDialog(this, "Ingrese el nombre de la nueva Droga CMV :", "Confirmar", 3);
		if(nueva != null){
			if(!nueva.equalsIgnoreCase("")|| !nueva.equalsIgnoreCase(" ")){
				CMVDrogas m = new CMVDrogas();
				m.setNombre(nueva);
				if(this.lista.contains(m)){
					JOptionPane.showMessageDialog(this,"Ese tipo de Droga de CMV ya se encuentra ingresado" , "Error", 2);
				}else{
					Fachada.getInstancia().guardarDrogaCMV(m);
				}
			}
		}
	}
	...
	private void modificar(){
		CMVDrogas m = (CMVDrogas) this.jListMedicacion.getSelectedValue();
		if(m!= null){
			JOptionPane.showMessageDialog(this,"Todas las complicaciones que esten asociados con esta Droga CMV se veran afectados" , "Atencion", 2);
			String nueva = "";
			nueva = JOptionPane.showInputDialog(this, "Ingrese el nuevo nombre de la Droga CMV :", "Confirmar", 3);
			if(nueva != null){
				if(!nueva.equalsIgnoreCase("")|| !nueva.equalsIgnoreCase(" ")){
					CMVDrogas aux = new CMVDrogas();
					aux.setNombre(nueva);
					if(this.lista.contains(aux)){
						JOptionPane.showMessageDialog(this,"La Droga CMV que quiere ingresar ya existe" , "Error", 2);
					}else{
						m.setNombre(nueva);
						Fachada.getInstancia().guardarDrogaCMV(m);
					}
	...
	private void eliminar(){
		...
		if(m!= null){
			String respuesta = "";
			respuesta = JOptionPane.showInputDialog(this, "Está a punto de eliminar una Droga de CMV\n(Ingrese SI para eliminar) Confirmar: ", "Confirmar", 1);
			if(respuesta==null)respuesta="No";
			if(respuesta.equalsIgnoreCase("Si")){
				boolean eliminar = Fachada.getInstancia().verificarEliminacionCMVDroga(m);
				if(!eliminar){
					JOptionPane.showMessageDialog(this,"Existe por lo una CMV con esta Droga CMV.\nPor lo tanto no se puede borrar." , "Error", 2);
				}else{
					Fachada.getInstancia().eliminarDrogaCMV(m);
				}
			}
		}
	}
