...
public class ControlDatosFijos {
	...
	public void crearPacienteDummie(){
		...
		g.setSql(sql);
		g.guardar();
		sql = "INSERT INTO trasplante(PreTrasplante,FECHA,NUM_TR,ID_DONANTE,INESTAB_HEMOD,RINHON,ANOMALIA_VASC,NUM_ART,NUM_VENAS,URETER,NUM_COMP_A_B,NUM_COMP_DR,NUM_IMCOMP_A_B,NUM_IMCOMP_DR,AUTOAC,PRA_MAX,PRA_TR,TRANS_PREVIAS,NUM_TRANSF,EMBARAZO,NUM_EMBARAZO,LIQ_PERFUSION,TQ_DE_BANCO,LADO_IMPLANTE,ANAST_VENOSA,ANAST_ARTERIAL,ANAST_URETERAL,T_ISQ_CAL_MIN,T_ISQ_FRIA_HS,T_ISQ_FRIA_MIN,T_ISQ_TIBIA_HS,REPERCUSION,SANGRADO_I_OP,LESION_ARTERIAL,LESION_VENOSA,NECESIDAD_REPEFUNDIR,OTRAS_COMPL_QUIRUR,DIURESIS_I_OP,CR_INICIAL,DIA_REC_DIURESIS,DIA_REC_FUNCIONAL,DIALISIS,NUM_DE_HD,COMENTARIO,FECHA_ALTA) VALUES ((select count(*) from pacientepretrasplante),'1900-01-01',1,' ',false,' ',false,1,1,' ',1,2,3,0,false,' ',' ',false,0,'null',0,' ',false,' ',' ',' ',' ',0,0,0,0,' ',false,false,false,false,' ',true,0,0,0,false,0,' ',null);";
		g.setSql(sql);
		g.guardar();
		sql = "COMMIT;";
		g.setSql(sql);
